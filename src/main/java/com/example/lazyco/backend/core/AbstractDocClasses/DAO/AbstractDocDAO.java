...
public class AbstractDocDAO<D extends AbstractDTO<D>, E extends AbstractModel>
  ...
  private MongoCriteriaBuilderWrapper getCriteriaBuilderWrapper(
      Class<E> filterClass, D filter, BiConsumer<MongoCriteriaBuilderWrapper, D> addEntityFilters) {
  ...
  protected void commonAbstractDTOUnauditedFilters(
      MongoCriteriaBuilderWrapper criteriaBuilderWrapper) {
  ...
  private void addIdInFilter(MongoCriteriaBuilderWrapper criteriaBuilderWrapper) {
    if (Objects.isNull(criteriaBuilderWrapper.getFilter().getIdsIn())
        || criteriaBuilderWrapper.getFilter().getIdsIn().isEmpty()) {
      return;
    }
  ...
  private void addIdNotInFilter(MongoCriteriaBuilderWrapper criteriaBuilderWrapper) {
    if (Objects.isNull(criteriaBuilderWrapper.getFilter().getIdsNotIn())
        || criteriaBuilderWrapper.getFilter().getIdsNotIn().isEmpty()) {
      return;
    }
